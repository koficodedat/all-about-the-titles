...
router.route('/')
	.post( async (rq, rs) => {
		const result = await searchTitle(rq.body.criteria); //todo:
		rs.status(200).json(result.rows);
	});

router.route('/detail')
	.post( async (rq, rs) => {
		const result = await getDetail(rq.body.id); //todo:
		rs.status(200).json(result.rows[0]);
	})

router.route('/all')
	.post( async (rq, rs) => {
		const result = await all('Casablanca'); //todo:
		rs.status(200).json(result.rows);
	})
